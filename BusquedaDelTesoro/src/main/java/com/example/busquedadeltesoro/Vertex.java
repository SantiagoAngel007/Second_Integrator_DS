package com.example.busquedadeltesoro;

import GraphicElements.ItemType;
import Graphs.ColorType;
import javafx.scene.control.Label;
import javafx.scene.paint.Color;
import javafx.scene.shape.Circle;
import java.util.ArrayList;
import java.util.List;

public class Vertex<K extends Comparable<K>> implements Comparable<K>{
    private K key;
    private ColorType color;
    private Vertex<K> previous;
    private int distance;
    private int time;
    List<Vertex<K>> connectedVertex;
    private Circle circulo;
    private ItemType item;
    private String name;
    private double x;
    private double y;
    private static final double RADIUS = 10;

    public Vertex(K key, ArrayList<Vertex<K>> connectedVertex) {
        this.key = key;
        color = ColorType.WHITE;
        this.connectedVertex = connectedVertex;
    }

    public Vertex(K key, ArrayList<Vertex<K>> connectedVertex, double x, double y, String name, ItemType item){
        this.name = name;
        this.x = x;
        this.y = y;
        this.key = key;
        color = ColorType.WHITE;
        this.connectedVertex = connectedVertex;
        this.item = item;
        //circulo = new Circle(x, y, 10, graphicColor);
    }

    Circle getCircle() {
        Circle circle;
        if(item == ItemType.INITIAL){
            circle = new Circle(x, y, RADIUS, Color.CHOCOLATE);
        }else {
            circle = new Circle(x, y, RADIUS, Color.BLUE);
        }
//        Circle circle = new Circle(x, y, RADIUS, Color.BLUE);
        return circle;
    }

    public Label getNameLabel() {
        Label nameLabel = new Label(name);
        nameLabel.setLayoutX(getX()+10);
        nameLabel.setLayoutY(getY()-15);
        nameLabel.setTextFill(Color.BLACK);
        return nameLabel;
    }

    public void setConnectedVertex(List<Vertex<K>> connectedVertex) {
        this.connectedVertex = connectedVertex;
    }

    public void setCirculo(Circle circulo) {
        this.circulo = circulo;
    }

    public String getName() {
        return name;
    }

    public void setName(String name) {
        this.name = name;
    }

    public double getX() {
        return x;
    }

    public void setX(double x) {
        this.x = x;
    }

    public double getY() {
        return y;
    }

    public void setY(double y) {
        this.y = y;
    }

    public ItemType getItem() {
        return item;
    }

    public void setItem(ItemType item) {
        this.item = item;
    }

    public Circle getCirculo() {
        return circulo;
    }

    public K getKey() {
        return key;
    }

    public void setKey(K key) {
        this.key = key;
    }

    public int getDistance() {
        return distance;
    }

    public void setDistance(int distance) {
        this.distance = distance;
    }

    public int getTime() {
        return time;
    }

    public void setTime(int time) {
        this.time = time;
    }

    public ColorType getColor() {
        return color;
    }

    public void setColor(ColorType color) {
        this.color = color;
    }

    public Vertex<K> getPrevious() {
        return previous;
    }

    public void setPrevious(Vertex<K> previous) {
        this.previous = previous;
    }

    public List<Vertex<K>> getConnectedVertex() {
        return connectedVertex;
    }

    public void setConnectedVertex(ArrayList<Vertex<K>> connectedVertex) {
        this.connectedVertex = connectedVertex;
    }

    @Override
    public int compareTo(K o) {
        return key.compareTo(o);
    }
}
